...
本文档描述如何在 Tauri 2 项目中接入 `corex-daemon` Daemon，实现全局快捷键截图、系统托盘等功能。

**架构约束：** Tauri **不得**依赖 `corex-core` crate。所有重逻辑由 sidecar `corex-daemon.exe` 承担，Tauri 仅通过 Unix socket 发送 JSON。
...
sequenceDiagram
    ...
    Tauri->>Sidecar: spawn sidecar --socket
...
| 组件 | 位置 | 依赖 |
|------|------|------|
| corex-daemon | sidecar 二进制 | xcap, image, tokio 等 |
...
1. 已构建 `corex-daemon.exe`：`cargo build -p corex-daemon --release`
2. Tauri 2 项目（含 `tauri-plugin-shell`、`tauri-plugin-global-shortcut`）
3. Windows 开发环境（Unix socket）
...
| `examples/tauri/scripts/copy-corex-daemon.mjs` | 项目根 `scripts/` |
...
src-tauri/
  binaries/
    corex-daemon-x86_64-pc-windows-msvc.exe
```
...
脚本逻辑见 `examples/tauri/scripts/copy-corex-daemon.mjs`：读取 `rustc --print host-tuple`，复制并重命名。
...
- Tauri 运行时自动解析为 `corex-daemon-{target-triple}.exe`
...
1. `spawn_corex_sidecar` — `app.shell().sidecar("binaries/corex-daemon")`
...
### Pipe 连接失败：`无法连接 corex.sock`

**原因：** Daemon 未运行或 Pipe 名称不匹配。

**解决：**

1. 确认 `corex-daemon` 已启动
2. 确认 Tauri 与 Daemon 使用相同 `--socket` 参数
...
1. 检查 `src-tauri/binaries/corex-daemon-x86_64-pc-windows-msvc.exe` 是否存在
2. 重新运行 `copy-corex-daemon.mjs`
3. 确认 `externalBin` 为 `"binaries/corex-daemon"`（逻辑名）

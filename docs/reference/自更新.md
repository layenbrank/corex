...
| `check_interval_hours` | `24`                   | 两次后台检查的最小间隔（`0` = 每次都查）                   |
...
| `timeout_secs`         | `30`                   | 连接建立与单次读取的超时（**不是**整次传输的上限）         |
...
- 后台检查按 `check_interval_hours` 节流，并缓存 `ETag` 做条件请求：命中 `304` 时无需重新解析响应（GitHub 对**已认证**的条件请求不计入主限流，匿名请求不保证豁免）。

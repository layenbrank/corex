...
| `text.case`                           | `act-text`        | `text`、`mode` (`upper` \| `lower` \| `title`)                                                                                                                                                 | 大小写变换；`title` 以非字母数字为词边界，保留原空白                                            |
| `text.join`                           | `act-text`        | `parts` (array)；`sep?` (`""`)                                                                                                                                                                 | 用分隔符拼接数组 → 字符串                                                                       |
| `text.split`                          | `act-text`        | `text`、`sep`（非空）；`limit?` (0 = 不限段数)                                                                                                                                                 | 拆成数组；`limit` 是段数上限，多余部分并入最后一段                                              |
| `text.replace`                        | `act-text`        | `text`、`from`；`to?` (`""`)、`regex?`                                                                                                                                                         | 字面 / 正则替换；正则的 `to` 支持 `$1`                                                          |
| `text.trim`                           | `act-text`        | `text`；`mode?` (`both` \| `start` \| `end`)                                                                                                                                                   | 去两端空白                                                                                      |
| `text.match`                          | `act-text`        | `text`、`pattern`                                                                                                                                                                              | 正则匹配 → `{matched,groups}`；`groups[0]` 是整体匹配，未参与组为 `null`                        |
| `math.eval`                           | `act-math`        | `expr`                                                                                                                                                                                         | 求值一个表达式（MiniJinja 语法，结果须是标量）                                                  |
| `math.rand`                           | `act-math`        | `min?` (0)、`max?` (100)、`int?` (true)                                                                                                                                                        | 在 `[min,max]` 内取随机数；`int` 为真取整数，否则浮点                                           |
| `math.round`                          | `act-math`        | `n`；`mode?` (`round` \| `floor` \| `ceil` \| `trunc`)                                                                                                                                         | 取整 → 整数                                                                                     |
| `math.stats`                          | `act-math`        | `values` (array)                                                                                                                                                                               | 统计数字数组 → `{sum,min,max,mean,count}`；非数值项报错并指出下标                               |
| `time.parse`                          | `act-time`        | `text`；`format?` (`""` = 自动识别)、`utc?`                                                                                                                                                    | 解析 → `{iso8601,unix}`；吃 RFC3339 / unix 秒 / `YYYY-MM-DD`，或给 `format`                     |
| `time.format`                         | `act-time`        | `at`；`format?` (`%Y-%m-%d %H:%M:%S`)、`utc?`                                                                                                                                                  | 按 `format` 输出字符串；`utc` 为真按 UTC，否则按本机时区                                        |
| `time.add`                            | `act-time`        | `at`；`days?`、`hours?`、`minutes?`、`seconds?`                                                                                                                                                | 时刻加偏移 → `{iso8601,unix}`                                                                   |
| `time.sub`                            | `act-time`        | `at`；`days?`、`hours?`、`minutes?`、`seconds?`                                                                                                                                                | 时刻减偏移 → `{iso8601,unix}`                                                                   |
| `time.tz`                             | `act-time`        | `at`、`offset`（`+08:00` \| `-0530` \| `utc`）                                                                                                                                                 | 换算到 `offset` 时区 → `{iso8601,unix}`；同一瞬间 unix 秒不变                                   |
...
| `dialog.choice`                       | `act-sys`         | `prompt`、`options`；`title?`                                                                                                                                                                  | 单选对话框：选项排成一列单选按钮，返回 `{ok,index,value}`                                       |
| `dialog.open` / `save`                | `act-sys`         | `title?`、`filter?`、`initial_dir?`；save 另有 `default_name?`                                                                                                                                 | 系统「打开 / 另存为」对话框；取消时 `ok=false`、`path` 为空                                     |
| `url.open`                            | `act-sys`         | `url`                                                                                                                                                                                          | ShellExecute 打开                                                                               |
| `process.list` / `kill`               | `act-sys`         | list:`name_contains?`；kill:`pid`                                                                                                                                                              | 进程枚举/结束                                                                                   |
| `app.launch`                          | `act-sys`         | `path` \| `aumid`；`args?`、`cwd?`、`wait?`、`if_running?`                                                                                                                                     | 启动程序 / 商店应用（aumid 经 `explorer.exe` 转交）；默认派发即返回                             |
| `sys.lock`                            | `act-sys`         | —                                                                                                                                                                                              | 锁定工作站（Windows）                                                                           |
| `sys.sleep`                           | `act-sys`         | `ms`                                                                                                                                                                                           | 异步等待毫秒（不占 `ui_settle_limit`）                                                          |

...
[`WatchEngine`](../../crates/engine/src/watch/engine.rs) 单路径处理 notify 事件。时序为 **debounce → throttle**（可组合，职责分离）：

```text
FS 事件 ──debounce(debounce_ms)──► 触发信号 ──throttle(throttle_ms)──► run_directive
```

```mermaid
flowchart LR
    Debouncer["debouncer_full\n(FS quiet debounce)"] --> Classify[event classify]
    Classify --> Filter[WatchFilter glob]
    Filter --> Throttle["worker throttle\n(leading+trailing)"]
    Throttle --> Run[run_directive_file]
    Err[Rescan or debouncer Err] --> Remount[remount paths]
```

- **Debounce（`debounce_ms`）**：`notify_debouncer_full` 的 **文件系统安静期** 合并抖动；不是 lodash debounce API，也不是 worker 内再叠一层假 debounce
- **Throttle（`throttle_ms`）**：限制 pipeline 执行频率（lodash 默认边沿：`leading: true, trailing: true`）
  - 窗口外首次触发：**立即** run（leading）；窗口从 **invoke 开始** 计时
  - 窗口内再有触发：标记 pending，窗口结束时 **最多再 run 一次**（trailing）
  - 窗口内仅 leading、之后无新事件：**不**额外 trailing
  - run 很长时 trailing 与 `is_running` 不重叠（等当前跑完再 trailing / CAS 互斥）
...
- **`immediate` / `RUN_NOW`**：立刻跑一次，并刷新 throttle 的 `last_invoke`（避免紧接着又 leading 一次）
...
| `[logging]` | 级别 / JSON 日志 |
| `[runtime]` | `max_parallel`、`step_timeout_secs`、`strict_permissions`、`filesystem_roots`、`ui_profile` / `ui_max_selector_chain` / `ui_max_settle_ms`、`cron_timezone` |
| `[update]` | `enabled`、`channel`、`repository` / `api_base_url`、`token_env`、`use_system_proxy`、`check_on_start` / `check_interval_hours`、`require_confirmation`、`timeout_secs` |

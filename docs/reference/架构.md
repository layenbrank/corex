...
| 层级     | 旋钮                                              | 作用域            | 默认 | 说明                                                               |
| -------- | ------------------------------------------------- | ----------------- | ---- | ------------------------------------------------------------------ |
| 请求之间 | `[daemon] max_jobs`                               | daemon 收到的请求 | `1`  | `1` 串行 / `> 1` 上限 / `0` 不限；只限 `run_directive` / `invoke`  |
...
| 场景                                   | 建议                                                                                           |
| -------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `ui.*` / `capture.*` 驱鼠标键盘        | **保持串行**：指令内别并发这些步骤，daemon 也保持 `max_jobs = 1`——两条指令同时驱输入必然互相踩 |
...
| 段          | 用途                                                                                                                                                         |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `[daemon]`  | `socket_path`、`lock_path`、`token`（及 `COREX_TOKEN` / token 文件）、`max_jobs`（同时**执行**的请求数：`1` 串行 / `0` 不限）                                |

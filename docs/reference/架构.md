...
- **占位符**：`{{var}}`、`{{input.x}}`、`{{env.X}}`、`{{step.id}}`（见 [directive-yaml.md](指令YAML.md)）。动作参数在执行前统一解析这一层，只有声明 `raw_params` 的动作（如 `template.render`）例外——它自己解释模板语法，绕过预解析。
- **控制流**：`if` / `repeat` / `parallel`。
  - **`parallel`**：写死的一组分支并发跑；有效并发度（步骤 `max_concurrency` 或配置 `runtime.max_parallel`）**> 1** 且分支多于 1 个时使用 `buffer_unordered`，否则顺序执行。
  - **`repeat`**：`max_concurrency` 与 `repeat` 同级 —— 省略 / `1` 串行（元素共享一份上下文，前一个的写入后一个看得见），`> 1` 并发（每个元素一份上下文副本，跑完按元素顺序合并）。
...
并发有**两个层级**，旋钮不通用：指令**内部**的步骤并发（引擎）与指令**之间**的请求并发（daemon）。
把两者混起来调是常见误配。

| 层级     | 旋钮                                              | 作用域            | 默认 | 说明                                                               |
| -------- | ------------------------------------------------- | ----------------- | ---- | ------------------------------------------------------------------ |
| 请求之间 | `[daemon] max_jobs`                               | daemon 收到的请求 | `4`  | `1` 串行 / `> 1` 上限 / `0` 不限；只限 `run_directive` / `invoke`  |
| 步骤之间 | 步骤级 `max_concurrency`（`parallel` / `repeat`） | 该块              | —    | 与 `repeat` / `parallel` **同级**，不是嵌套子键                    |
| 步骤之间 | `[runtime] max_parallel`                          | 全进程            | `8`  | `parallel` 块的有效并发度上界（步骤没写 `max_concurrency` 时用它） |
| 步骤之间 | `corex run --jobs N`                              | 单次运行          | —    | 只覆盖本次的 `parallel` 最大并发，不落盘                           |
| 单步     | `[runtime] step_timeout`                          | 全进程            | `0`  | 秒；`0` = 不限                                                     |
| 单步     | `corex run --timeout SECS`                        | 单次运行          | —    | 只覆盖本次，不落盘                                                 |

控制类请求（`ping` / `shutdown` / `list_*`）**不排 `max_jobs` 的队**——它们是宿主判断
「daemon 还活着吗」的手段，被一条几分钟的指令堵住是最糟的结果。连接本身也总是并发的。

`max_jobs` 默认 `4`（不再串行）：宿主的「多任务并跑」是常态，串起来只会让后面几条一直排队。
排队的请求**每两秒收到一帧心跳**，客户端据此把「在队列里等」与「卡死」分开——排队期间一条
步骤帧也不会有，只按「多久没有帧」判死必然把长排队误报成失败（而请求之后照样执行）。

### 串行与并发的语义差别（关键）

`repeat` 的 `max_concurrency` 不只是「快慢」，它**改变可见性**：

| `max_concurrency`  | 上下文                    | 元素之间                                       |
| ------------------ | ------------------------- | ---------------------------------------------- |
| 省略 / `1`（串行） | 所有元素**共享一份** ctx  | 前一个元素的写入，后一个**看得见**             |
| `> 1`（并发）      | 每个元素**一份 ctx 副本** | 元素之间**互相看不见**；跑完按**元素顺序**合并 |

所以「累积计数」「先建目录再写文件」这类依赖前序写入的指令，**不能**靠加并发提速——并发化
等于换了一套语义。`parallel` 只跑写死的一组分支，每个分支各拿一份 ctx 副本、按序号合并。

有效并发度 **> 1 且分支多于 1 个**时才走 `buffer_unordered`，否则顺序执行（见 `Pipeline`）。
单个分支的执行顺序不保证，**归位靠 `id` / 序号**，不要靠完成先后。

### 怎么选

| 场景                                   | 建议                                                                                           |
| -------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `ui.*` / `capture.*` 驱鼠标键盘        | **显式串行**：指令内别并发这些步骤，daemon 也写 `max_jobs = 1`——两条指令同时驱输入必然互相踩   |
| 文件复制 / 下载 / 纯 IO                | 并发有明显收益；`repeat.each` 配 `max_concurrency: 4` 起步，按盘与带宽再调                     |
| 步骤间有数据依赖（后一步读前一步产物） | 串行，或把依赖显式写进 `params` 而不是靠 ctx 传递                                              |
| 想压满 CPU 的重计算                    | 注意 Action 内部是否已经并行；外层再叠并发只会互相抢                                           |
| 宿主同时要跑多条指令（互不干扰的）     | 调 daemon `max_jobs`，而不是把多条指令塞进一条里                                               |

**超时**是单步而不是整条指令的：`step_timeout` 到点后该步失败，随后按 `on_error` 走
（`abort` / `continue` / `skip`）。权限拒绝例外——它**始终中止**，且并行分支同时失败时优先保留
（见 [退出码与错误码](./退出码与错误码.md#3-权限拒绝的特殊地位)）。

语义由回归测试锁住：`repeat_serial_shares_context_but_concurrent_isolates_it`（同一份计数逻辑，
串行得 `123`、并发得 `3`）。
...
| 段          | 用途                                                                                                                                                         |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `[daemon]`  | `socket_path`、`lock_path`、`token`（及 `COREX_TOKEN` / token 文件）、`max_jobs`（同时**执行**的请求数：`1` 串行 / 默认 `4` / `0` 不限）                        |

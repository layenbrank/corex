...
可组合的快捷指令（Shortcut）/ Action 运行时：**YAML 定义流水线**，内置与 WASM 插件提供动作，**CLI 与 `corex-daemon` 共用同一引擎**。
...
| `examples/shortcuts/` | — | Shortcut 示例 |
...
# 运行示例 Shortcut
cargo run -p corex -- run examples/shortcuts/hello.yaml
cargo run -p corex -- run examples/shortcuts/hello.yaml --input who=Corex

# 列出 / 校验 / 创建
cargo run -p corex -- list
cargo run -p corex -- actions
cargo run -p corex -- validate examples/shortcuts/hello.yaml
cargo run -p corex -- create my-shortcut
...
示例 `hello.yaml` 会渲染问候语并写入 `/tmp/corex-hello.txt`（见 [`examples/shortcuts/hello.yaml`](examples/shortcuts/hello.yaml)）。另有 [`control-flow.yaml`](examples/shortcuts/control-flow.yaml)、[`copy-demo.yaml`](examples/shortcuts/copy-demo.yaml)。

## CLI 命令

| 命令 | 说明 |
|------|------|
| `corex run <name\|path>` | 执行 Shortcut（`--input KEY=VALUE`） |
| `corex list` | 列出 Shortcut |
| `corex actions` | 列出已注册 Action |
| `corex create <name>` | 创建 Shortcut 脚手架 |
| `corex validate <path>` | 校验 YAML |
| `corex repl` | 交互：`help` / `actions` / `list` / `run` / `quit` |
| `corex daemon` | `start` / `stop` / `status` / `run` |

全局可用 `-v` / `-vv` 提高日志级别；`--dir` 覆盖 shortcuts 目录。
...
- YAML：Pipeline v3 → **Shortcut**（`action` + `{{ }}`）；旧样例在 [`examples/legacy/`](examples/legacy/)。
- 旧 `corex copy` / `pipeline` / `watch` 子命令已移除；用 Action（如 `copy.run`）写进 Shortcut，或 IPC `invoke`。

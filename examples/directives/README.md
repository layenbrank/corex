...
| [upload-chunked.yaml](./upload-chunked.yaml)         | 大文件分片上传：`generate.chunks` 切块 + `generate.hash` 算摘要 → PATCH 绑定（秒传判定）→ `http.send` multipart 逐片（带断点续传） | `corex run upload-chunked -i file=D:/big.bin`             |
